...
rip_charset = ascii_letters


def gen_printable_sequence(limit=10, charset=rip_charset):
    seq = chain.from_iterable((product(*([charset] * i))
...
def gen_checks(md5hex, variant_sequence):
    for variant in variant_sequence:
        ...
        yield rip(variant, md5hex), variant
        if match:
            break
...
def main():
    ...
    time1 = time.time()
    pool_cpu_time1 = map(lambda p: map(
        float,
    ...
    pool_cpu_time2 = map(lambda p: map(
        float,
        get_stat_info(p.pid)[13:15]
    ), pool._pool)
